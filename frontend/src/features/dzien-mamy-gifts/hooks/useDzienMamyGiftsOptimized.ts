...
import { DzienMamyApi } from '../api/dzien-mamy.api';
import { DzienMamyFilters, Product } from '../types';

interface UseDzienMamyGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseDzienMamyGiftsOptions {
...
const EMPTY_FILTERS = {} as DzienMamyFilters;

export default function useDzienMamyGiftsOptimized({
  filters = EMPTY_FILTERS,
  initialPerPage = 16,
  autoLoad = true
}: UseDzienMamyGiftsOptions = {}): UseDzienMamyGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await DzienMamyApi.fetchDzienMamyGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Dzień Mamy gifts:', err);
      setError('Nie udało się załadować prezentów na Dzień Matki');
