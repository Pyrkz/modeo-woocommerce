...
import DzienTatyEmptyState from './DzienTatyEmptyState';
import { Product } from '../types';

interface DzienTatyGiftsGridProps {
  products: Product[];
  loading?: boolean;
  error?: string | null;
...
const DzienTatyGiftsGrid = memo(({
    ...
    return (
      <div className={className}>
        ...
        <div className="text-center mt-8">
          <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-full">
            <svg className="w-5 h-5 mr-3 text-blue-600 animate-spin" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="m4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 714 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span className="text-sm text-gray-700">🎁 Wczytujemy prezenty dla taty...</span>
          </div>
        </div>
      </div>
    );
  ...
  if (products.length === 0 && !loading) {
    return (
      <div className={className}>
        <DzienTatyEmptyState onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      ...
      {loading && products.length > 0 && (
        <div className="text-center mt-8">
          <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-full">
            <svg className="w-4 h-4 mr-3 text-blue-600 animate-spin" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="m4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 714 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
...
DzienTatyGiftsGrid.displayName = 'DzienTatyGiftsGrid';

export default DzienTatyGiftsGrid;