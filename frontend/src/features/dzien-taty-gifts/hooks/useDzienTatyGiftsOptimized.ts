...
import { DzienTatyApi } from '../api/dzien-taty.api';
import { DzienTatyFilters, Product } from '../types';

interface UseDzienTatyGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseDzienTatyGiftsOptions {
...
const EMPTY_FILTERS = {} as DzienTatyFilters;

export default function useDzienTatyGiftsOptimized({
  filters = EMPTY_FILTERS,
  initialPerPage = 16,
  autoLoad = true
}: UseDzienTatyGiftsOptions = {}): UseDzienTatyGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await DzienTatyApi.fetchDzienTatyGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Dzień Taty gifts:', err);
      setError('Nie udało się załadować prezentów na Dzień Ojca');
