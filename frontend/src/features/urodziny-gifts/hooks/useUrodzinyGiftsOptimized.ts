...
import { UrodzinyFilters, UrodzinyProduct } from '../types';

interface UseUrodzinyGiftsResult {
...
interface UseUrodzinyGiftsOptions {
  filters?: UrodzinyFilters;
  initialPerPage?: number;
  autoLoad?: boolean;
}

export default function useUrodzinyGiftsOptimized({
  filters = {},
  initialPerPage = 16,
  autoLoad = true
}: UseUrodzinyGiftsOptions = {}): UseUrodzinyGiftsResult {
  const [products, setProducts] = useState<UrodzinyProduct[]>([]);
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await UrodzinyApi.getProducts(
        page,
        perPageRef.current,
        filtersRef.current
      );
      ...
      console.error('Failed to load Urodziny gifts:', err);
      setError('Nie udało się załadować prezentów na urodziny');
