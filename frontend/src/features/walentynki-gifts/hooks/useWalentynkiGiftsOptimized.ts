...
import { WalentynkiApi } from '../api/walentynki.api';
import { WalentynkiFilters, Product } from '../types';

interface UseWalentynkiGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseWalentynkiGiftsOptions {
...
const EMPTY_FILTERS = {} as WalentynkiFilters;

export default function useWalentynkiGiftsOptimized({
  filters = EMPTY_FILTERS,
  initialPerPage = 16,
  autoLoad = true
}: UseWalentynkiGiftsOptions = {}): UseWalentynkiGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await WalentynkiApi.fetchWalentynkiGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Walentynki gifts:', err);
      setError('Nie udało się załadować prezentów walentynkowych');
