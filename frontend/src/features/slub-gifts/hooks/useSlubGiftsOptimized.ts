...
import { SlubApi } from '../api/slub.api';
import { SlubFilters, Product } from '../types';

interface UseSlubGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
const EMPTY_FILTERS = {} as SlubFilters;

export default function useSlubGiftsOptimized({
  filters = EMPTY_FILTERS,
  initialPerPage = 16,
  autoLoad = true
}: UseSlubGiftsOptions = {}): UseSlubGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await SlubApi.fetchSlubGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Ślub gifts:', err);
      setError('Nie udało się załadować prezentów ślubnych');
