...
import { SylwesterApi } from '../api/sylwester.api';
import { SylwesterFilters } from '../types';
import { Product } from '@/types/product';

interface UseSylwesterGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseSylwesterGiftsOptions {
  filters?: SylwesterFilters;
  initialPerPage?: number;
  autoLoad?: boolean;
}

export default function useSylwesterGiftsOptimized({
  filters = {},
  initialPerPage = 16,
  autoLoad = true
}: UseSylwesterGiftsOptions = {}): UseSylwesterGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await SylwesterApi.fetchSylwesterGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Sylwester gifts:', err);
      setError('Nie udało się załadować prezentów sylwestrowych');
