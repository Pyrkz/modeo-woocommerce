...
import { ChristmasApi } from '../api/christmas.api';
import { ChristmasFilters, Product } from '../types';

interface UseChristmasGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseChristmasGiftsOptions {
...
const EMPTY_FILTERS = {} as ChristmasFilters;

export default function useChristmasGiftsOptimized({
  filters = EMPTY_FILTERS,
  initialPerPage = 16,
  autoLoad = true
}: UseChristmasGiftsOptions = {}): UseChristmasGiftsResult {
  ...
  const loadGifts = useCallback(async (
    ...
    try {
      ...
      const response = await ChristmasApi.fetchChristmasGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Christmas gifts:', err);
      setError('Nie udało się załadować prezentów świątecznych');
