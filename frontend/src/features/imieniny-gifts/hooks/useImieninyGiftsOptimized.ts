...
import { ImieninyApi } from '../api/imieniny.api';
import { ImieninyFilters } from '../types';
import { Product } from '@/types/product';

interface UseImieninyGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseImieninyGiftsOptions {
  filters?: ImieninyFilters;
  initialPerPage?: number;
  autoLoad?: boolean;
}

export default function useImieninyGiftsOptimized({
  filters = {},
  initialPerPage = 16,
  autoLoad = true
}: UseImieninyGiftsOptions = {}): UseImieninyGiftsResult {
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      const response = await ImieninyApi.fetchImieninyGifts(
        filtersRef.current,
        page,
        perPageRef.current
      );
      ...
      console.error('Failed to load Imieniny gifts:', err);
      setError('Nie udało się załadować prezentów na imieniny');
