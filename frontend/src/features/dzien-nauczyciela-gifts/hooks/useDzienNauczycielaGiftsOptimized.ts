...
interface UseDzienNauczycielaGiftsResult {
  products: Product[];
  loading: boolean;
  error: string | null;
  total: number;
...
interface UseDzienNauczycielaGiftsOptions {
  filters?: ShopFilters;
  initialPerPage?: number;
  autoLoad?: boolean;
}

export default function useDzienNauczycielaGiftsOptimized({
  filters = {},
  initialPerPage = 16,
  autoLoad = true
}: UseDzienNauczycielaGiftsOptions = {}): UseDzienNauczycielaGiftsResult {
  ...
  // Always include dzien-nauczyciela-specific search terms
  const memoizedFilters = useMemo(() => {
    return {
      search: 'teacher nauczyciel education school edukacja szkoła prezent', // Always search for teacher terms
  ...
  const loadProducts = useCallback(async (
    ...
    try {
      ...
      console.error('Failed to load dzień nauczyciela gifts:', err);
      setError('Nie udało się załadować prezentów na Dzień Nauczyciela');
  ...
  }, [memoizedFilters]); // Empty dependency array - function is stable
